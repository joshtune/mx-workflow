...
Where the project has an established convention it follows that. Where there's no precedent, it falls back to the [Project Structure](/mx-workflow/reference/project-structure/) and [Code Style](/mx-workflow/reference/code-style/) rules — lowest-common-ancestor file placement, no sibling-subtree imports, folder-qualified filenames, strings extracted to constants, guard clauses over nesting, no speculative abstraction. Any deviation is reported in its output with the rule, the reason, and the rejected alternative.
