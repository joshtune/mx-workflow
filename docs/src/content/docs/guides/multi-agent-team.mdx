...
1. Implementation agents complete tasks as normal
...
| **Test coverage** | Every implemented feature has a corresponding test (e2e for user flows, unit for business logic). Reports which features are missing tests. |
...
:::tip[QA agent is role-agnostic]
The QA agent works with any team composition. It does not need to understand the domain — it runs the project's existing quality commands and verifies contracts. A frontend/backend team, a microservices team, or a DevOps/infra team all get the same verification protocol.
:::
