...
When you have a product or feature idea and want Claude to handle the entire journey — discovery questions, PRD, plan, build, QA, and commit. Auto-selects agent team or single-agent based on plan complexity. Makes incremental commits at each phase.
...
When you want more control over each step. Use the PRD to define scope, then spawn a multi-agent team build manually. This gives you direct access to each agent. See the [Multi-Agent Team guide](/mx-workflow/guides/multi-agent-team/) for details.
...
For any feature from idea to code. Asks discovery questions, generates PRD and plan, builds (auto-selects agent team or single-agent), runs QA, and commits — all in one command.
